...
operations
    ...
    tick()
        begin
            --Propaga un tick de reloj a los segmentos para que ellos los 
            --propaguen a los trenes que se encuentran en ellos.
        ...
        post algunoHabraSalidoSiHabiaTrenesOperativos: 
            (self@pre.proximaSalida()->notEmpty()) implies 
                (self.proximaSalida() <> self@pre.proximaSalida())
    llegaTren(t:Tren)
        begin
...
operations
    ...
    -- Devuelve el proximo tren que saldrá, o null si no hay tren o todavía no ha 
    -- llegado al final (similar a un peek)
...
operations
    ...
    siguienteEstacion():Estacion =
        ...
        in
            if 
                (linea.estacion -> indexOf(e1) > linea.estacion -> indexOf(e2) xor 
    ...
    siguienteVia(): Via =
        ...
        then
            self.estacion.segmento->any(
                s | s.estacion->includes(self.linea.siguiente(self.estacion)) and 
                s.linea->includes(self.linea)
            ).via->any(v | v.sentido = self.sentido)
        else
            self.estacion.segmento->any(
                s | s.estacion->includes(self.linea.anterior(self.estacion)) and 
                s.linea->includes(self.linea)
                ).via->any(v | v.sentido = self.sentido)
        endif
    saleEstacion()
        ...
        pre estacionCorrespondeLinea: 
            siguienteVia().segmento.estacion->
            includes(self.estacion)
    ...
    llegaEstacion()
        begin
            ...
            if 
                self.estacion = self.linea.cabezaDeLinea() or 
                self.estacion = self.linea.finalDeLinea()
            then
                self.darLaVuelta();
            end;
        end
        pre enVia: self.via->size() = 1
        pre viaConectaEstacion: self.via.segmento.estacion->
            includes(siguienteEstacion())
