...
Para este apartado hemos aplicado el Patrón **Estado** y el Patrón **Mediador** para poder cambiar dinámicamente en tiempo de ejecución el tipo de Semáforo.

Para ello partimos del apartado anterior añadiendo la clase `MediadorBiestableTriestable`, que tiene un atributo referenciando a la clase abstracta `Semaforo`, y la interfaz `Mediador`. Además, `Semaforo` añade los métodos `setMediador()`, `getEstado()`, `setEstado()` y un constructor, todos ellos de visibilidad protegida.

De esta manera, el cliente se comunicará con el dispositivo a través de la clase `MediadorBiestableTriestable`, que se encargará de cambiar el estado del dispositivo según el Semáforo que sea y cambiará de tipo de Semáforo a través del método `cambiar()`, que llamará al método `notificar()`, implementada de la interfaz `Mediador`, que a su vez llamará a `cambiarABiestable()` o `cambiarATriestable()` dependiendo del Semáforo que sea.
