...
cumpla los requisitos establecidos. Por último, el método `rutina4` será solo
...
**Estado** que se asemeja notablemente con lo descrito en la práctica.

Para ello tenemos un objeto de la clase `Biestable` que representa el contexto
del Patrón **Estado** determina el comportamiento del biestable en las funciones
`abrir()` y `cerrar()`. La función `estado()` devuelve un mensaje indicando en
qué estado se encuentra. Además tiene un atributo del tipo `EstadoBiestable`,
que es la interfaz que representa los diferentes estados.

En este caso la interfaz es implementada por dos clases que representan los dos
posibles estados de un biestable: `Rojo` y `Verde`.
