...
del Patrón **Estado** determina el comportamiento del biestable en las 
funciones `abrir()` y `cerrar()`. La función `estado()` devuelve un mensaje 
indicando en qué estado se encuentra. Además tiene un atributo del tipo 
`EstadoBiestable`, que es la interfaz que representa los diferentes estados.
...
llamada `Semaforo`. Decidimos hacer que fuera una clase abstracta en vez de 
una interfaz ya que necesitamos que sus métodos `abrir()` y `cerrar()` a su 
vez sean abstractos y cada dispositivo lo implemente como vea necesario.
...
De la misma manera, partimos del apartado anterior añadiendo la clase 
`Contexto` que tiene su constructor, los métodos `abrir()`, `cerrar()`, 
`estado()` y `cambio()` y un atributo del tipo `Semaforo` que a su vez 
representa el contexto de la clase `estadoSemaforo`.

Es decir, hemos aplicado el Patrón **Estado** y el Patrón **Estrategia**.

El cambio más significativo de este apartado ha sido la función `cambio()`, 
que ha implicado que nos planteásemos las condiciones de cambio de dispositivo 
en tiempo de ejecución de Biestable a Triestable y viceversa. De Biestable a 
...
Es decir, a un dispositivo Triestable en estado `Amarillo` al que le 
aplicásemos la función `cambio()` no haría nada ni cambiaría de estado.
...
llame al método `cambio()` desde la clase `Contexto` se le pase por parámetro 
el estado en el que se encuentra y si es posible el cambio, el cual una vez es 
