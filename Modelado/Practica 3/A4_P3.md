...
instancia de la clase Acceso. El resto de la interfaz publica está compuesta por
_getters_.
...
método estático `crearPaciente()` para generar nuevas instancias. Esto es asi
...
expediente inicial y asociarlo al paciente.
...
```{include=src/test/Ej1.java}
```

### Test Ejercicio 2

```{include=src/test/Ej2.java}
```

### Test Ejercicio 3

```{include=src/test/Ej3.java}
```